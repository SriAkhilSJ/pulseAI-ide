...
export class BrowserTitlebarPart extends Part implements ITitlebarPart {
	...
	protected override createContentArea(parent: HTMLElement): HTMLElement {
		...
		// Agent Status Indicator (PulseCode AI)
		const agentStatus = $('div.pulse-agent-status');
		agentStatus.textContent = '🟢 Agent connected';
		agentStatus.style.color = '#a3a3a3';
		agentStatus.style.fontSize = '12px';
		agentStatus.style.padding = '4px 10px';
		agentStatus.style.marginRight = '10px';
		agentStatus.style.borderRadius = '12px';
		agentStatus.style.border = '1px solid #333';
		agentStatus.style.display = 'flex';
		agentStatus.style.alignItems = 'center';
		agentStatus.style.cursor = 'pointer';
		append(this.rightContent, agentStatus);
